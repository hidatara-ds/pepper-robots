...
def index():
    ...
    <body>
        ...
        <button id="micBtn" title="Tap to recognitionrd"><span id="micIcon">🎤</span></button>
        <div id="response"></div>
        <div id="log"></div>
        <script>
        let mediaRecorder;
        ...
        async function startRecording() {
            ...
            try {
                ...
                mediaRecorder.start();
                isRecording = true;
                micBtn.classList.add('recognitionrding');
        ...
        function stopRecording() {
            if (mediaRecorder && isRecording) {
                mediaRecorder.stop();
                isRecording = false;
                micBtn.classList.remove('recognitionrding');
                micIcon.textContent = '🎤';
                log('Stop recognitionrding requested');
            }
        }
...
def api_process_audio():
    try:
        ...
        response = speech_client.recognitiongnize(config=config, audio=audio)
...
def recognitionrd_audio(duration=5, fs=16000):
...
def main():
    ...
    while True:
        ...
        audio, fs = recognitionrd_audio(duration=5)
